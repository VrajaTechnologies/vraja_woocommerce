...
class ProductCategory(models.Model):
    ...
    display = fields.Selection([('default', 'Default'), ('products', 'products'),('subcategories', 'Subcategories'),('both', 'Both')],
                                             string='Display', default='default')
    ...
    def export_product_category_wizard(self):
        return {
            'name': 'Export Product Category To Woocommerce',
            'type': 'ir.actions.act_window',
            'res_model': 'woocommerce.export.product.category',
            'view_mode': 'form',
            'target': 'new',
            'context': {
                'active_ids': self.ids,
    ...
    def search_or_create_product_category(self, instance, product_category_response, log_id):
        ...
        odoo_parent = self.search([('code', '=', parent_id), ('instance_id', '=', instance.id)], limit=1) if parent_id else False
        display = product_category_response.get('display')
        ...
        if not product_category_id:
            self.create({'name': category_name, 'code': category_id, 'instance_id': instance.id,
                         'company_id': instance.company_id.id, 'slug': slug, 'parent_id': odoo_parent.id if odoo_parent else False, 'display': display})
            msg = "product category Successfully Created {}".format(category_name)
        else:
            msg = "product category Already exist {}".format(category_name)
